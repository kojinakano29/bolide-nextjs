...
const ModalCosmeticBrand = ({ show, close, cosmetics, setCosmetics }) => {
    ...
    const handleCheckCosmeBrand = useCallback(
        e => {
            if (cosmetics?.includes(e.target.value)) {
                setCosmetics(cosmetics.filter(item => item !== e.target.value))
            } else {
                                        ...
                                        {item.group.map(gr => (
                                            <label key={gr.name}>
                                                <input
                                                    ...
                                                    onChange={
                                                        handleCheckCosmeBrand
                                                    }
                                                    checked={cosmetics?.includes(
                                                        gr.name,
                                                    )}
                                                />
                                                {gr.name}
