...
const CanDo = () => {
  ...
  const handleClickTab = useCallback(async (num) => {
    setCurrent(num)
  }, [setCurrent])
    ...
    <article className={styles.cont2__flex}>
            ...
            {canDoLeft.map((can, index) => (
              ...
              >
                ...
                <div className={styles.cont2__item__right}>
                  ...
                  {current === index + 1 ?
                    <p className={styles.cont2__item__desc}>{can.txt}</p>
                  : null}
                </div>
              </Scroll>
            ))}
      ...
      {canDoImage.map((img, index) => (
        <div
          ...
          className={`${styles.cont2__right} ${current === index + 1 ? styles.current : null}`}
          key={index}
        >
          <Image
            src={img.url}
