...
const Release = ({data, detail = false, swiper = false}) => {
  ...
  return (
    <>
      <a href={`/corapura/press_release/${data.id}`} className={`${styles.releaseLink} ${swiper ? styles.swiper : null}`}>
        <div className={styles.imgBox}>
          <img src={data.thumbs ? `${process.env.NEXT_PUBLIC_BACKEND_URL}/storage/${data.thumbs}` : dummy.src} alt="プレスリリースのサムネイル画像" />
        </div>
      ...
      {!detail ?
        <a href={`/corapura/${data?.user?.account_type === 0 ? 'influencer' : 'company'}/${data?.user?.id}`} className={`${styles.company} hoverEffect`}>
          <div className={styles.logoBox}>
            {data.user.c_profile.thumbs ? <img src={`${process.env.NEXT_PUBLIC_BACKEND_URL}/storage/${data.user.c_profile.thumbs}`} alt="プロフィール画像" /> : null}
          </div>
