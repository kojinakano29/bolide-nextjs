...
const MatterDetail = ({ posts }) => {
    ...
    const onLoadCheck = async () => {
        ...
        await axios
            .post('/api/corapura/post_app/check', {
                user_id: user?.id,
            })
            .then(res => {
                // console.log(res)
    ...
    const onMatterStatus = async () => {
        ...
        await axios
            .post(`/api/corapura/post_app/list`, {
                c_post_id: posts.id,
            })
            .then(res => {
                // console.log(res)
    ...
    const handleClickMatterAdd = useCallback(async () => {
        ...
        await axios
            .post(`/api/corapura/post_app/add`, {
                c_post_id: posts.id,
                user_id: user?.id,
    ...
    const handleClickFinish = useCallback(async () => {
        ...
        await axios
            .post(`/api/corapura/post/compleate/${posts.id}`, {
                state: 1,
            })
            .then(res => {
                // console.log(res)
    ...
    const handleClickBookmark = useCallback(async () => {
        ...
        if (bookmark?.includes(posts.id)) {
            await axios
                .delete('/api/corapura/post_bookmark/delete', {
                    data: {
                        user_id: user?.id,
                        c_post_id: posts.id,
                    },
            ...
            await axios
                .post('/api/corapura/post_bookmark/store', {
                    user_id: user?.id,
                    c_post_id: posts.id,
                })
                .then(res => {
    ...
    const handleChangeState = useCallback(
        async (e, current) => {
            setState(
                state.map((st, index) =>
                    index === current ? parseInt(e.target.value) : parseInt(st),
                ),
            )
        },
        [state, setState],
    ...
    const handleClickState = useCallback(
        async (status, id, name) => {
            ...
            if (parseInt(status) === parseInt(5)) {
                await axios
                    .delete(`/api/corapura/post_app/delete`, {
                        data: {
                            app_id: id,
                        },
                    })
                ...
                await axios
                    .post(`/api/corapura/post_app/state_change/${id}`, {
                        state: status,
                    })
                    .then(res => {
                        // console.log(res)
                        ...
                        <div className={styles.headRight}>
                            ...
                            <ul className={styles.tags}>
                                {posts.c_tags.map((tag, index) => (
                                    <li className={styles.tag} key={index}>
                                        <a
                                            href={`/corapura/${
                                                posts.user.account_type === 0
                                                    ? 'influencer'
                                                    : 'company'
                                            }/matter/?tag_id=${tag.id}`}
                            ...
                            <a
                                ...
                                <div className={styles.logoBox}>
                                    {posts.user.c_profile.thumbs ? (
                                        <img
                                            src={`${process.env.NEXT_PUBLIC_BACKEND_URL}/storage/${posts.user.c_profile.thumbs}`}
                                            alt="プロフィール画像"
                                        />
                                    ) : null}
                                </div>
                                        ...
                                        <div
                                            ...
                                            <div className={styles.left}>
                                                <div className={styles.imgBox}>
                                                    {list.c_profile.thumbs ? (
                                                        <img
                                                            src={`${process.env.NEXT_PUBLIC_BACKEND_URL}/storage/${list.c_profile.thumbs}`}
                                                            alt="プロフィール画像"
                                                        />
                                                    ) : null}
                                                </div>
