...
const CompanyCard = ({ data }) => {
    ...
    return (
        <a
            href={`/corapura/company/${data?.id}`}
            className={`hoverEffect ${styles.companyBox}`}>
            <div className={styles.company__top}>
                <div className={styles.company__topLeft}>
                    <img
                        src={
                            data?.c_profile?.image1
                                ? `${process.env.NEXT_PUBLIC_BACKEND_URL}/storage/${data?.c_profile?.thumbs}`
                                : dummy.src
                        }
                        alt="プロフィール画像"
                    />
