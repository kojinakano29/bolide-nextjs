...
const Faq = ({faqs}) => {
  ...
  const handleClickFaq = useCallback(async (num) => {
    if (num === current) {
      setCurrent(null)
      return
    }
      ...
      <Container small900>
        ...
        <div className={styles.faq}>
          {before.map((faq, index) => (
            <dl className={current === index ? styles.current : null} key={index}>
          ...
          {more ?
            after.map((faq, index) => (
              <dl className={current === index+4 ? styles.current : null} key={index+4}>
