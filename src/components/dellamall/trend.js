...
const Trend = ({ mb40 = false, sp }) => {
    ...
    const onMount = useCallback(async () => {
        ...
        await axios
            .post(`/api/dellamall/tag/list`)
            .then(res => {
                // console.log(res)
                setTrends(
                    res.data.filter((tag, index) => {
                        return (
                            index < 20 ||
                            parseInt(router.query.tag_id) === parseInt(tag.id)
                        )
                    }),
                )
            })
    ...
    return (
        <>
            <p className={`${styles.title} ${mb40 ? styles.mb40 : ''}`}>
                TREND
            </p>
            {trends.length === 0 ? (
                <Loader />
                ...
                <div className={sp ? styles.sp_slide : null}>
                    <ul className={styles.trend__list}>
                        {trends?.map((trend, index) => (
                            <li
                                className={`${styles.keyWord__item} ${
                                    trend.id === parseInt(router.query.tag_id)
                                        ? styles.current
                                        : null
                                }`}
                                key={index}>
