...
const SaveMall = () => {
    ...
    const loadMall = async () => {
        ...
        await axios
            .post('/api/dellamall/shop/mall_return', {
                user_id: user?.id,
                d_shop_id: shop.id,
            })
            .then(res => {
    ...
    const handleClickSaveMallToggle = useCallback(
        async (mallId, mallName) => {
            ...
            if (mallIn?.includes(mallId)) {
                await axios
                    .delete('/api/dellamall/mall_in/delete', {
                        data: {
                            d_mall_id: mallId,
                            d_shop_id: shop.id,
                            user_id: user?.id,
                ...
                await axios
                    .post('/api/dellamall/mall_in/store', {
                        d_mall_id: mallId,
                        d_shop_id: shop.id,
                        user_id: user?.id,
                    })
    ...
    return (
        <>
            <div
                            ...
                            mallList?.map((list, index) => (
                                <button
                                    className={`${styles.saveMallBox__btn} ${
                                        mallIn?.includes(list.id)
                                            ? styles.saveOn
                                            : null
                                    }`}
                                    key={index}
                                    onClick={() =>
                                        handleClickSaveMallToggle(
                                            list.id,
                                            list.name,
                                        )
                                    }>
                                    <div className={styles.imgBox}>
                                        {list.d_mall_in?.[0]?.thumbs &&
                                        list.d_mall_in?.[0]
                                            ?.image_permission === 1 ? (
                                            <img
                                                src={`${process.env.NEXT_PUBLIC_BACKEND_URL}/storage/${list.d_mall_in?.[0]?.thumbs}`}
                                                alt="モールの画像"
                                            />
                                        ) : null}
                                    </div>
                                    <span className={styles.saveMallBox__name}>
                                        {list.name}
                                    </span>
                ...
                <a
                    className={styles.saveMallBox__create}
            ...
            {popupOpen ? (
                <CreateMallContext.Provider
                    value={{ handleClickPopup, setMallList, user, shop }}>
                    <CreateMall />
                </CreateMallContext.Provider>
            ) : null}
        </>
