...
export const getServerSideProps = async ({ params }) => {
    const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_CORAPURA}/user/show/${params.id}`,
    )
                    ...
                    <div className={styles.appealBox}>
                        <div className={styles.left}>
                            ...
                            <p className={styles.desc}>
                                {profile.c_user_profile.appeal_text}
                            </p>
                        </div>
                        <div className={styles.imgBox}>
                            <img
                                src={
                                    profile.c_user_profile.appeal_image
                                        ? `${process.env.NEXT_PUBLIC_BACKEND_URL}/storage/${profile.c_user_profile.appeal_image}`
                                        : null
                                }
                                alt="アピールの画像"
                            />
