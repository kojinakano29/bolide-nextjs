...
const EditUser = ({ posts }) => {
    ...
    const [previewAppeal, setPreviewAppeal] = useState(
        option.appeal_image
            ? `${process.env.NEXT_PUBLIC_BACKEND_URL}/storage/${option.appeal_image}`
            : '',
    )
    ...
    const onUserProfileUpdate = useCallback(
        async data => {
            ...
            await axios
                .post(
                    `/api/corapura/mypage/c_user_profile/update/${option.id}`,
                    params,
                    {
                        headers: {
                            'Content-Type': 'multipart/form-data',
    ...
    const handleClickEdit = async (type, id) => {
        ...
        if (type === 'card') {
            ...
            await axios
                .post(`/api/corapura/mypage/c_user_social/edit/${id}`)
                .then(res => {
                    // console.log(res)
                    setEditData(res.data)
                })
    ...
    const handleClickDelete = async (type, id) => {
        ...
        if (type === 'card') {
            ...
            await axios
                .delete(`/api/corapura/mypage/c_user_social/delete`, {
                    data: {
                        c_user_social_id: id,
                        c_profile_id: profile.id,
                    },
