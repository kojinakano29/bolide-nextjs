...
export const getServerSideProps = async ({ params }) => {
    const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_CORAPURA}/post/edit/${params.id}`,
    )
...
const EditMatter = ({ posts }) => {
    ...
    const {
        ...
        defaultValues: {
            ...
            number_of_people: post?.number_of_people
                ...
                : null,
            recruitment_quota: post?.recruitment_quota
                ? post.recruitment_quota
                : null,
            speciality: post?.speciality ? post.speciality : null,
            suporter: post?.suporter ? post.suporter : null,
            amount_of_suport: post?.amount_of_suport
                ? post.amount_of_suport
                : null,
    ...
    const onMatterEdit = useCallback(
        async data => {
            await csrf()
            ...
            await axios
                .post(`/api/corapura/post/update/${post.id}`, params, {
                    headers: {
                        ...
                        <form onSubmit={handleSubmit(onSubmit)}>
                                ...
                                <div className={styles.matterRight}>
                                    ...
                                    <dl>
                                        <dt>
                                            ...
                                            <GuidePopup
                                                txt={`投稿する案件の大まかな内容を表すようなフレーズを入れてタイトルを設定しましょう。\n例）モデル募集！見習いスタイリストの練習モデルになってくれませんか？`}
                                            />
                                        </dt>
                                        <dd>
                                            <input
                            ...
                            <article className={styles.selectorArea}>
                                ...
                                selector === 12 ? (
                                    <dl>
                                        <dt>
                                            <label htmlFor="recruitment_quota">
                                                募集可能枠
                                            </label>
                                        </dt>
                                        <dd>
                                ...
                                {selector === 17 ? (
                                    <dl>
                                        <dt>
                                            <label htmlFor="amount_of_suport">
                                                目標支援総額
                                            </label>
                                        </dt>
                                        <dd>
