...
export const getServerSideProps = async ({ params }) => {
    const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_CORAPURA}/mypost/compleate/${params.id}`,
    )
...
const EditorComment = ({ posts }) => {
    ...
    const onSubmit = useCallback(
        async data => {
            ...
            await axios
                .post(`/api/corapura/comment/store`, {
                    user_id: user?.id,
                ...
                .then(res => {
                    ...
                    axios
                        .post(
                            `/api/corapura/c_post_app/state_change_comment_compleate/${currentMatter.id}`,
                        )
                        .then(res => {
                            // console.log(res)
                            setMatters(res.data)
                            ...
                            {matters.map((matter, index) => (
                                <div className={styles.listItem} key={index}>
                                    <div className={styles.itemLeft}>
                                        <a
                                            href={`/corapura/matter/${matter.c_post.id}`}
