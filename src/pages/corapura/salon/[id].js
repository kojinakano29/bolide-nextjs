...
export const getServerSideProps = async ({ params }) => {
    const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_CORAPURA}/salon/show/${params.id}`,
    )
...
const OnlineSalonDetail = ({ posts }) => {
    ...
    const onLoadCheck = async () => {
        ...
        await axios
            .post('/api/corapura/salon_bookmark/check', {
                user_id: user?.id,
            })
            .then(res => {
                // console.log(res)
        ...
        await axios
            .post(`/api/corapura/salon_app/check`, {
                user_id: user?.id,
                c_salon_id: salon.id,
            })
            .then(res => {
        ...
        await axios
            .get(`/api/subscription/status/${user?.id}/salon${salon.id}`)
            .then(res => {
    ...
    const handleClickFollow = useCallback(async () => {
        ...
        if (followCheck) {
            await axios
                .delete('/api/corapura/salon_bookmark/delete', {
                    data: {
                        user_id: user?.id,
                        c_salon_id: salon.id,
                    },
            ...
            await axios
                .post('/api/corapura/salon_bookmark/store', {
                    user_id: user?.id,
                    c_salon_id: salon.id,
                })
                .then(res => {
    ...
    const handleClickPlan = useCallback(async () => {
        ...
        if (planCheck) {
            ...
            await axios
                .delete(`/api/corapura/salon_app/delete`, {
                    data: {
                        user_id: user?.id,
                        c_salon_id: salon.id,
                    },
            ...
            router.push(
                `/payment/${user?.id}/?plan=salon&salon_id=${
                    salon.id
                }&salon_plan=${salon.stripe_api_id}&type=${
                    resume ? 'plan_change' : 'subscribe'
                }`,
            )
        }
