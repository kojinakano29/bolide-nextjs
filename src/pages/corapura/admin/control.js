...
const AdminControl = () => {
    ...
    const onSubmitStripeId = async data => {
        ...
        await axios
            .post(`/api/coprapura/admin/salon/update/${data.c_salon_id}`, {
                stripe_api_id: data.stripe_api_id,
            })
            .then(res => {
                // console.log(res)
                alert(
                    `ID${res.data.id}のオンラインサロンにStripeのプランIDを追加しました`,
                )
                setPopup(false)
            })
                                ...
                                <dl>
                                    ...
                                    <dd>
                                        <input
                                            type="text"
                                            id="stripe_api_id"
                                            {...register('stripe_api_id', {
                                                required: true,
                                            })}
                                            placeholder="StripeのプランIDを入力してください"
                                        />
                                        {errors.stripe_api_id && (
                                            <p className={styles.error}>
                                                必須項目を入力してください
                                            </p>
                                        )}
                ...
                <Container small900>
                    ...
                    <div className={styles.linkFlex}>
                        <a
                            ...
                            href="/corapura/admin/matter/?type=company">
                            企業案件一覧
                        </a>
                        <a
                            className="hoverEffect"
                            href="/corapura/admin/matter/?type=user">
                            インフルエンサー/ユーザー案件一覧
                        </a>
                        <a
                            className="hoverEffect"
                            href="/corapura/admin/press_release">
                            プレスリリース一覧
                        </a>
                        <a className="hoverEffect" href="/corapura/admin/salon">
                            オンラインサロン一覧
                        </a>
                    </div>
                </Container>
