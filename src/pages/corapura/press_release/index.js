...
const PressReleaseList = ({ posts }) => {
    ...
    }, [setReleases, setNowPage, setMaxPage, search, tag, sort, page])
                        ...
                        <article className={styles.prList}>
                            {releases.map((release, index) => (
                                <div className={styles.prItem}>
                                    ...
                                    <a
                                        href={`/corapura/${
                                            release.user.account_type === 0
                                                ? 'influencer'
                                                : 'company'
                                        }/${release.user.id}`}
                                        className={`${styles.company} hoverEffect`}>
                                        <div className={styles.logoBox}>
                                            {release.user.c_profile.thumbs ? (
                                                <img
                                                    src={`${process.env.NEXT_PUBLIC_BACKEND_URL}/storage/${release.user.c_profile.thumbs}`}
                                                    alt="プロフィール画像"
                                                />
                                            ) : null}
                                        </div>
