...
export const getServerSideProps = async ({ params }) => {
    const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_LIONDOR}/faq/edit/${params.id}`,
    )
...
const FaqEdit = ({ posts }) => {
    ...
    const onPostForm = useCallback(async data => {
        ...
        await axios
            .post(`/api/liondor/faq/update/${post.id}`, data)
            .then(res => {
                // console.log(res)
                alert('更新しました。')
                setDisabled(false)
...
FaqEdit.getLayout = function getLayout(page) {
