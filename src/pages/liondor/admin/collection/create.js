...
export const getServerSideProps = async () => {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_LIONDOR}/admin/collection/create`)
...
const CreateCollection = ({posts}) => {
  ...
  const onPostForm = useCallback(async (data) => {
    ...
    await axios.post('/api/liondor/admin/collection/store', params, {
      headers: {
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <article className={styles.flex}>
              <div className={styles.left}>
                ...
                {errors.image1 || errors.image2 || errors.image3 || errors.image4 ? <p className={`red ${styles.error}`}>サムネイル画像を全て入力してください</p> : null}
              </div>
