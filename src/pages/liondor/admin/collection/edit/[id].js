...
export const getServerSideProps = async ({ params }) => {
    const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_LIONDOR}/admin/collection/edit/${params.id}`,
    )
...
const CreateCollection = ({ posts }) => {
    ...
    const [preview1, setPreview1] = useState(
        post.image1
            ? `${process.env.NEXT_PUBLIC_BACKEND_URL}/storage/${post.image1}`
            : '',
    )
    const [preview2, setPreview2] = useState(
        post.image2
            ? `${process.env.NEXT_PUBLIC_BACKEND_URL}/storage/${post.image2}`
            : '',
    )
    const [preview3, setPreview3] = useState(
        post.image3
            ? `${process.env.NEXT_PUBLIC_BACKEND_URL}/storage/${post.image3}`
            : '',
    )
    const [preview4, setPreview4] = useState(
        post.image4
            ? `${process.env.NEXT_PUBLIC_BACKEND_URL}/storage/${post.image4}`
            : '',
    )
    const previews = [
    ...
    const onPostForm = useCallback(
        async data => {
            ...
            await axios
                .post(
                    `/api/liondor/admin/collection/update/${post.id}`,
                    params,
                    {
                        headers: {
