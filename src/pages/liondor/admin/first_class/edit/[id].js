...
export const getServerSideProps = async ({params}) => {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_LIONDOR}/admin/first/edit/${params.id}`)
...
const EditFirstClass = ({posts}) => {
  ...
  const [preview, setPreview] = useState(`${process.env.NEXT_PUBLIC_BACKEND_URL}/storage/${post.thumbs}`)
  ...
  const onPostForm = useCallback(async (data) => {
    ...
    await axios.post(`/api/liondor/admin/first/update/${post.id}`, params, {
      headers: {
  ...
  const handleChangeFile = useCallback((e) => {
    ...
    if (files[0]) {
      ...
      setPreview(`${process.env.NEXT_PUBLIC_BACKEND_URL}/storage/${preview}`)
    }
  }, [setPreview, preview])
              ...
              <div className={styles.left}>
                ...
                <dl className={styles.dl}>
                  ...
                  <dd className={styles.dd}>
                    ...
                    {preview ? <img src={preview ? preview : null} alt="プレビュー画像" /> : null}
                  </dd>
                </dl>
              </div>
