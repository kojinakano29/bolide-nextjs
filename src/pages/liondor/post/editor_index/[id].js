...
export const getServerSideProps = async ({ params }) => {
    const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_LIONDOR}/post/editor_index/${params.id}`,
    )
...
const PostList = ({ posts }) => {
    ...
    const handleClickCat = useCallback(
        async (id, name) => {
            await csrf()

            await axios
                .post(`/api/liondor/post/editor_index/${user?.id}`, {
                    l_category_id: id,
                    page: 1,
                })
                .then(res => {
    ...
    const onClickDelete = async post => {
        ...
        if (userIdStr === postStr) {
            await axios
                .delete(`/api/liondor/post/delete/${post.id}`)
                .then(res => {
                    // console.log(res)
                    alert('記事を削除しました。')
                    router.reload()
                ...
                <Container small900>
                    ...
                    <div className={styles.bigCatBox}>
                        <button
                            ...
                            className={`${styles.bigCat} ${
                                catName === 'All' ? styles.current : null
                            }`}
                        ...
                        {bigCat.map((cat, index) => (
                            <button
                                type="button"
                                className={`${styles.bigCat} ${
                                    cat.slug === catName ? styles.current : null
                                }`}
                    ...
                    <div className={styles.smCatBox}>
                        {catArray[catName].map((cat, index) => (
                            <button
                                type="button"
                                className={`${styles.smCat} ${
                                    cat.id === currentId ? styles.current : null
                                }`}
                                onClick={() =>
                                    handleClickCat(cat.id, cat.parent_slug)
                                }
                                key={index}>
                                {cat.name}
                            </button>
                        ...
                        <ul>
                            {lists?.map((list, index) => (
                                <li key={index}>
                                    ...
                                    <a
                                        href={`/liondor/post/show/${list.id}`}
                                    ...
                                    <div className={styles.btnBox}>
                                        <a
                                            href={`/liondor/post/edit/${list.id}`}
                                            className={styles.edit}>
                                            編集
                                        </a>
                                        <button
                                            className={styles.delete}
                                            onClick={() => onClickDelete(list)}
                                            disabled={disabled}>
                                            削除
                                        </button>
                                        {list.state === 0 ? (
                                            <a
                                                href={`/liondor/post/show/${list.id}/?preview=1`}
                                                className={styles.edit}
                                                target="_blank"
                        ...
                        <div className="pagerBox">
                            {nowPage === 1 ? null : (
                                <button
                                    ...
                                    onClick={() =>
                                        handleClickPage(nowPage - 1)
                                    }></button>
                            )}
                            <p className="pagerCurrent en">
                                {nowPage}/{maxPage}
                            </p>
                            {maxPage === nowPage ? null : (
                                <button
