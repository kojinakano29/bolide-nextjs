...
export const getServerSideProps = async ({ params }) => {
    const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_LIONDOR}/post/edit/${params.id}`,
    )
...
const PostEdit = ({ posts }) => {
    ...
    const onPostForm = useCallback(async data => {
        ...
        await axios
            .post(`/api/liondor/post/update/${post.id}`, params, {
                headers: {
    ...
    const [preview, setPreview] = useState(
        post.thumbs ? defaultThumbsPreview : null,
    )
    ...
    const handleClickDeleteImage = useCallback(
        async type => {
            ...
            await axios
                .post(`/api/liondor/post/image_destroy/${post.id}`, {
                    type: type,
                })
                .then(res => {
                    // console.log(res)
        ...
        [post, setPreview, setPreview2, setDefaultThumb, setDefaultMv],
    )
    ...
    return (
        <section className={styles.createSection}>
                                        ...
                                        {preview && !preview.match(/null/) ? (
                                            <div className={styles.previewBox}>
                                                ...
                                                <button
                                                    ...
                                                    className={`${styles.imgDelete} hoverEffect`}>
                                                    ✕
                                                </button>
                                            </div>
                                        ) : null}
            ...
            <Button2
                link={`/liondor/post/editor_index/${user?.id}`}
                name="一覧へ戻る"
                noto
            />
        </section>
