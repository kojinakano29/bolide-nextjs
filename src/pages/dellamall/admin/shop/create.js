...
const CreateShop = () => {
    ...
    const onShopCreate = useCallback(
        async data => {
            await csrf()

            await axios
                .post('/api/dellamall/shop/store', data)
                .then(res => {
                    // console.log(res)
                    alert('ショップを作成しました。')
                    router.push({
    ...
    const handleClickSiteData = async url => {
        ...
        await axios
            .post('/api/dellamall/shop_create_url', {
                url: url,
            })
            .then(res => {
                // console.log(res)
                    ...
                    <form onSubmit={handleSubmit(onSubmit)}>
                                        ...
                                        <div className={styles.imgBox}>
                                            {preview ? (
                                                ...
                                                <div className={styles.imgNone}>
                                                    ショップオーナーからの申請が無い場合はキャプチャ画像は表示されません。ショップオーナーにキャプチャ画像が表示されるようにみんなで依頼しましょう！
                                                </div>
                                            )}
                                        </div>
                                    </dd>
                                ...
                                <dl>
                                    ...
                                    <dd>
                                        <textarea
                                            id="tag"
                                            {...register('tag')}
                                            placeholder="関連タグが自動反映されない場合は、タグを３つ入力してください"></textarea>
                                    </dd>
                                </dl>
                                <dl>
                                    <dt>
                        ...
                        <div className={styles.check}>
                            <label>
                                ...
                                />
                                <a href="/dellamall/terms" target="_blank">
                                    利用規約
                                </a>
                                に同意する
                            </label>
                            {errors.check && (
                                <p
                                    className={`orange ${styles.error} ${styles.error2}`}>
                                    ※こちらの項目は入力必須です
                                </p>
                            )}
                        </div>
