...
export const getServerSideProps = async ({ params }) => {
    const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_DELLAMALL}/shop/edit/${params.id}`,
    )
...
const EditShop = ({ posts }) => {
    ...
    const {
        ...
        defaultValues: {
            ...
            tag: tagStr,
            description:
                posts.description && posts.description !== 'undefined'
                    ? posts.description
                    : '',
        },
        mode: 'onChange',
    ...
    useEffect(() => {
        if (user) {
            ...
            if (posts?.official_user_id) {
                if (
                    parseInt(user?.id) !== parseInt(posts?.official_user_id) &&
                    user?.account_type < 3
                ) {
    ...
    const onShopEdit = useCallback(
        async data => {
            ...
            await axios
                .post(`/api/dellamall/shop/update/${posts.id}`, params, {
                    headers: {
    ...
    return (
        <section className={`${styles.adminForm} cont1`}>
            <Container small>
                ...
                user?.account_type > 2 ||
                user?.id === posts.official_user_id ? (
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <h3 className={styles.infoTtl}>基本情報</h3>
                        <p className={styles.desc}>
                            ※ショップ作成時の注意事項
                            <br />
                            ・既に同じURLのサイトがデラモールに登録されている場合は、重複したショップの作成はできません。
                            <br />
                            ・モール型ショップの場合は個々のショップではなく、モールショップのTOPのURLをお貼りください。
                            <br />
                            ・公式ショップ申請をされていない場合、サイトURL・関連タグ以外の情報は入力できません。
                        </p>
                                ...
                                <dl>
                                    <dt>
                                        ...
                                        <span>
                                            ...
                                            <br />
                                            ※公式ショップ登録したい方で、URLが登録できない場合にはお問合せください。
                                            <br />(
                                            <a href="/dellamall/contact">
                                                お問合せにリンク
                                            </a>
                                            )
                                        </span>
                                ...
                                <dl>
                                    ...
                                    <dd>
                                        <textarea
                                            ...
                                            disabled={officialCheck}></textarea>
                                    </dd>
                                </dl>
                            </div>
                        </article>
                        <div className={styles.btnFlex}>
                            ...
                            <a
                                href="/dellamall/admin/shop"
                                className={`${styles.btn} ${styles.btn3} hoverEffect`}>
                                戻る
                            </a>
                        </div>
                    </form>
