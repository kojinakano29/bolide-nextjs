        ...
        <div className={styles.signOutBox}>
          ...
          <div className={styles.txtBox}>
            ...
            <dl>
              ...
              <dd>
                退会手続き後、Bolide's Japan IDでご利用されているすべてのサービスの利用停止を行います。
                <br/>有料登録をされている場合は、「<a href={`/mypage/plan/${user?.id}`}>プラン確認</a>」画面より先にサブスクリプションの解約を行ってください。「<a href={`/mypage/option/${user?.id}`}>募金</a>」のサブスクリプションにも登録している方は「<a href={`/mypage/option/${user?.id}`}>募金</a>」画面よりサブスクリプションの解約を行ってください。
                <br/>サブスクリプションの解約を行わないまま、本ページで退会手続きを行うとサブスクリプション契約が継続された状態となってしまいます。
              </dd>
